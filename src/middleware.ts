...
import { GITHUB_API, PROTECTED_ROUTES, PUBLIC_ROUTES, ROUTES } from '@/constants/routes'
...
const middleware = async (req: NextRequest) => {
  ...
  if (path === ROUTES.LOGIN && session?.accessToken) {
    const { userName } = await fetchData<GitHubUserResponse>({ resource: GITHUB_API.ROUTES.USER })
